...
public class AccountController : Controller
{
    ...
    public async Task<IActionResult> Register([FromBody] RegisterDto user)
    {
        ...
        validator.Validate(new User(user.Login, user.Password, user.Name));
    ...
    {
        _logger.LogInformation($"Requested path: {HttpContext.Request.Path}");
        List<User> users = await _userRepository.GetAllUsers(Filter.ByLikes);
        ...
        {
            UserDto userDto = await _userDtoBuilder.CreateUserDto(user.Id);
    ...
    [Route("searchUsers/{filter:int}")]
    [HttpPost]
    public async Task<IActionResult> SearchUsers([FromRoute] int filter, [FromBody] SearchUserDto searchUserDto)
    {
        _logger.LogInformation($"Requested path: {HttpContext.Request.Path}");
        List<User> users = await _userRepository.SearchUsers((Filter)filter, searchUserDto.Name);
    ...
    public async Task<IActionResult> GetBestUser()
    {
        ...
        User user = await _userRepository.GetBestUser();

        if (user == null)
        {
            return BadRequest("Cannot get best user");
        }
